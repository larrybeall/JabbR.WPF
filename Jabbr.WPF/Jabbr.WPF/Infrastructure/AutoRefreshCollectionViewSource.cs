    ...
    //for the collection view to process unneeded events. This should just a bit of unneeded overhead.
        ...
        private void OnItemPropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            if (IsViewRefreshNeeded(e.PropertyName))
            {
                ...
                if (view != null)
                {
                    object current = view.CurrentItem;
        ...
        {
            return SortDescriptions.Any(sort => string.Equals(sort.PropertyName, propertyName)) ||
                   GroupDescriptions.OfType<PropertyGroupDescription>().Where(
                       g => string.Equals(g.PropertyName, propertyName)).Any();
        }
        ...
        protected class NotifyCollectionChangedWeakEventManager : WeakEventManager
        {
            ...
            public static NotifyCollectionChangedWeakEventManager CurrentManager
            {
                ...
                {
                    var manager =
                        (NotifyCollectionChangedWeakEventManager)
                        GetCurrentManager(typeof (NotifyCollectionChangedWeakEventManager));

                    if (manager == null)
                        SetCurrentManager(typeof (NotifyCollectionChangedWeakEventManager),
                                          (manager = new NotifyCollectionChangedWeakEventManager()));
        ...
        protected class NotifyPropertyChangedWeakEventManager : WeakEventManager
        {
            ...
            public static NotifyPropertyChangedWeakEventManager CurrentManager
            {
                ...
                {
                    var manager =
                        (NotifyPropertyChangedWeakEventManager)
                        GetCurrentManager(typeof (NotifyPropertyChangedWeakEventManager));

                    if (manager == null)
                        SetCurrentManager(typeof (NotifyPropertyChangedWeakEventManager),
                                          (manager = new NotifyPropertyChangedWeakEventManager()));
